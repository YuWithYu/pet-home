  ...
  <div class="litter-service-container">
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            ...
            <el-button type="primary" @click="showAddServiceDialog">添加寄养服务</el-button>
      ...
      <el-tabs v-model="activeTab" @tab-change="handleTabChange">
        <el-tab-pane label="所有寄养服务" name="all">
          <el-table 
            :data="services" 
            style="width: 100%"
            row-key="id"
        ...
        <el-tab-pane label="寄养服务订单" name="serviceOrders">
              ...
              <el-table-column label="服务项目" min-width="200">
                <template #default="{ row }">
                  <div v-for="product in row.products" :key="product.id" class="order-service">
                    ...
                    <el-tag size="small" style="margin-left: 8px;">寄养服务</el-tag>
                  </div>
                </template>
              </el-table-column>
      ...
      >
        ...
        <el-form-item label="服务分类" prop="category">
          <el-select v-model="serviceForm.category" placeholder="请选择服务分类" style="width: 100%">
            <el-option label="基础寄养服务" value="basic" />
            <el-option label="豪华寄养服务" value="luxury" />
            <el-option label="长期寄养服务" value="long-term" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="服务标签">
          ...
          >
            ...
            <el-option label="豪华住宿" value="豪华住宿" />
            <el-option label="专业护理" value="专业护理" />
            <el-option label="长期寄养" value="长期寄养" />
...
import boardingServiceApi from '@/api/boarding-service'
...
export default {
  ...
  setup() {
    ...
    // 上传配置
    const uploadUrl = ref('http://localhost:8080/api/boarding-services/upload')
    const bannerUploadUrl = ref('http://localhost:8080/api/boarding-banners/upload')
    ...
    const loadServices = async () => {
      ...
      try {
        ...
        const response = await boardingServiceApi.getBoardingServicePage(params)
        if (response.code === 0) {
          services.value = response.data.records || []
          total.value = response.data.total || 0
          console.log('加载寄养服务列表成功:', services.value)
        } else {
    ...
    const loadServiceBanner = async () => {
      try {
        const response = await boardingServiceApi.getServiceBanner()
        if (response.code === 0 && response.data) {
          const imageUrl = response.data.imageUrl
          if (imageUrl && !imageUrl.includes('default-boarding-banner.jpg')) {
    ...
    const saveService = async () => {
      try {
        ...
        if (isEdit.value) {
          ...
          response = await boardingServiceApi.updateBoardingService(data)
        } else {
          response = await boardingServiceApi.createBoardingService(data)
        }
    ...
    const deleteService = async (service) => {
      try {
        ...
        const response = await boardingServiceApi.deleteBoardingService(service.id)
        if (response.code === 0) {
    ...
    const toggleServiceStatus = async (service) => {
      try {
        ...
        const response = await boardingServiceApi.updateBoardingServiceStatus(service.id, newStatus)
