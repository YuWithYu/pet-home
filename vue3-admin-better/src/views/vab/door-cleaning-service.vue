  ...
  <div class="litter-service-container">
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            ...
            <el-button type="primary" @click="showAddServiceDialog">添加铲屎服务</el-button>
      ...
      <el-tabs v-model="activeTab" @tab-change="handleTabChange">
        <el-tab-pane label="所有铲屎服务" name="all">
          <el-table 
            :data="services" 
            style="width: 100%"
            row-key="id"
        ...
        <el-tab-pane label="铲屎服务订单" name="serviceOrders">
            ...
            >
              ...
              <el-table-column label="服务项目" min-width="200">
                <template #default="{ row }">
                  <div v-for="product in row.products" :key="product.id" class="order-service">
                    ...
                    <el-tag size="small" style="margin-left: 8px;">铲屎服务</el-tag>
                  </div>
                </template>
              </el-table-column>
              ...
              <el-table-column label="操作" width="150">
                ...
                <el-button type="text" @click.stop="viewOrderDetail(scope.row)">查看详情</el-button>
                  ...
                  <el-button 
                    v-if="scope.row.status === 'paid' || scope.row.status === 'confirmed'"
                    type="text" 
                    style="color: #409eff"
      ...
      >
        ...
        <el-form-item label="服务分类" prop="category">
          <el-select v-model="serviceForm.category" placeholder="请选择服务分类" style="width: 100%">
            <el-option label="基础铲屎服务" value="basic" />
            <el-option label="深度清洁服务" value="deep-cleaning" />
            <el-option label="定期维护服务" value="regular" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="服务标签">
          ...
          >
            ...
            <el-option label="快速清理" value="快速清理" />
            <el-option label="定期维护" value="定期维护" />
            <el-option label="除臭消毒" value="除臭消毒" />
          </el-select>
        </el-form-item>
      </el-form>
    ...
    >
      ...
      >
        ...
        <el-form-item label="最大预约数" prop="maxBookings">
          <el-input-number 
            v-model="timeSlotForm.maxBookings" 
            :min="1" 
            :max="100"
        ...
        <el-form-item label="状态" prop="isActive">
          <el-switch v-model="timeSlotForm.isActive" active-text="启用" inactive-text="禁用" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  setup() {
    ...
    // 上传配置
    const uploadUrl = ref('http://localhost:8080/api/litter-services/upload')
    const bannerUploadUrl = ref('http://localhost:8080/api/litter-banners/upload')
    ...
    const loadServices = async () => {
      ...
      try {
        ...
        const response = await litterServiceApi.getLitterServicePage(params)
        if (response.code === 0) {
          services.value = response.data.records || []
          total.value = response.data.total || 0
          console.log('加载铲屎服务列表成功:', services.value)
        } else {
    ...
    const loadServiceBanner = async () => {
      try {
        const response = await litterServiceApi.getServiceBanner()
        if (response.code === 0 && response.data) {
          const imageUrl = response.data.imageUrl
          if (imageUrl && !imageUrl.includes('default-litter-banner.jpg')) {
    ...
    const saveService = async () => {
      try {
        ...
        if (isEdit.value) {
          ...
          response = await litterServiceApi.updateLitterService(data)
        } else {
          response = await litterServiceApi.createLitterService(data)
        }
    ...
    const deleteService = async (service) => {
      try {
        ...
        const response = await litterServiceApi.deleteLitterService(service.id)
        if (response.code === 0) {
    ...
    const toggleServiceStatus = async (service) => {
      try {
        ...
        const response = await litterServiceApi.updateLitterServiceStatus(service.id, newStatus)
    ...
    // 订单相关方法（优先订单汇总接口，兜底预约分页）
    const loadServiceOrders = async () => {
      ordersLoading.value = true
      try {
        // 1) 主数据源：订单汇总接口
        let mapped = []
        const resOrders = await api.get('/api/orders/list', { params: { status: orderFilterStatus.value || '' } })
        if (resOrders && resOrders.code === 0 && resOrders.data) {
          mapped = (resOrders.data.orders || []).map(o => ({
            id: o.id,
            customer: o.customer,
            phone: o.phone,
            products: o.products || [],
            ...
            createTime: o.appointmentDate || o.createTime || ''
          }))
        }

        // 2) 兜底：如果为空，再用预约分页接口
        if (!mapped.length) {
          const resPage = await api.get('/api/door-cleaning/page', { params: { pageNo: 1, pageSize: 1000 } })
          if (resPage && resPage.code === 0 && resPage.data) {
            const records = resPage.data.records || []
            mapped = records.map(r => {
              const appointmentDate = r.date || r.appointmentDate || r.createTime || ''
              return {
                id: r.id,
                customer: r.contactName || `用户${r.userId || ''}`,
                phone: r.contactPhone || '',
                products: [{ id: r.id, name: r.serviceType || '上门铲屎服务' }],
                totalAmount: r.price || 0,
                status: r.status || 'pending',
                createTime: appointmentDate
              }
            })
          }
        }

        serviceOrders.value = mapped
        orderTotal.value = mapped.length
        if (!mapped.length) {
          ElMessage.info('暂无订单数据')
        }
      } catch (e) {
        console.error('加载铲屎服务订单失败:', e)
        serviceOrders.value = []
        orderTotal.value = 0
      } finally {
    ...
    const viewOrderDetail = async (order) => {
      try {
        ...
        if (res.code === 0) {
          ElMessage.info(`订单 ${order.id}，客户：${res.data.customer}，金额：¥${res.data.totalAmount}`)
        } else {
    ...
    const confirmOrder = async (order) => {
      try {
        const res = await api.put(`/api/door-cleaning/${order.id}/status`, null, { params: { status: 'confirmed' } })
        if (res.code === 0) {
    ...
    const completeOrder = async (order) => {
      try {
        const res = await api.put(`/api/door-cleaning/${order.id}/status`, null, { params: { status: 'completed' } })
        if (res.code === 0) {
    ...
    const saveTimeSlot = async () => {
      try {
        ...
        if (response.code === 0) {
          ElMessage.success(isEditSlot.value ? '更新成功' : '添加成功')
