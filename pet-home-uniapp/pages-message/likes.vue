<template>
  <view class="likes-container">
    ...
    <scroll-view class="content-list" scroll-y @scrolltolower="loadMore" enable-back-to-top>
      <view v-if="loading && likesList.length === 0" class="loading-state">
        <text>加载中...</text>
      </view>
      <view v-else-if="likesList.length === 0" class="empty-state">
        <view class="empty-icon">📦</view>
        <text class="empty-text">暂无互动消息</text>
        <text class="empty-hint">多发帖子可以收获赞和收藏~</text>
      </view>
      <view v-else>
        <view 
          class="like-item" 
        ...
        >
          ...
          <view class="like-content">
            <view class="like-header">
              ...
              <text class="like-time">{{ formatTime(item.createTime) }}</text>
            </view>
            <view class="like-action">
              <text class="action-text">{{ item.type === 'like' ? '赞了' : '收藏了' }}你的动态</text>
            </view>
...
export default {
  ...
  onShow() {
    ...
    if (this.currentUserId) {
      uni.setStorageSync('message_lastSeenLikesAt', Date.now())
    }
  },
  ...
  methods: {
    ...
    async loadLikes() {
      ...
      try {
        // 调用后端接口获取点赞和收藏列�?
        const res = await api.getLikesAndCollections({
        ...
        if ((res.code === 0 || res.code === 200) && res.data) {
          const newLikes = res.data.likes || res.data.records || res.data || []
          
          if (this.page === 1) {
            this.likesList = newLikes
          } else {
            this.likesList = [...this.likesList, ...newLikes]
          }
          
          this.hasMore = newLikes.length >= this.pageSize
        }
      } catch (error) {
        console.error('加载点赞和收藏列表失败:', error)
        // 如果接口不存在，显示空状�?
      } finally {
        this.loading = false
      }
