  ...
  <view class="booking-container">
    ...
    <view class="booking-form">
      ...
      <view class="form-section">
        ...
        <view class="store-selector" @click="openKeyPicker('handover')">
          <view class="selector-label">{{ formData.keyHandoverMethod || '请选择钥匙给予方式' }}</view>
        </view>
      </view>
      ...
      <view class="form-section">
        ...
        <view class="store-selector" @click="openKeyPicker('return')">
          <view class="selector-label">{{ formData.keyReturnMethod || '请选择钥匙归还方式' }}</view>
        </view>
      </view>
    ...
    <view v-if="showStorePicker" class="picker-modal store-picker-modal" @click="showStorePicker = false">
      <view class="picker-content store-picker-content" @click.stop>
        <view class="picker-header">
          <view class="picker-title">选择上门铲屎门店</view>
          <view class="picker-close" @click="showStorePicker = false">×</view>
        </view>
        <view class="picker-list">
          <view v-if="litterStores.length === 0" class="empty-stores-hint">暂无门店，请在管理后台「服务管理」-「服务门店管理」中添加</view>
          <view
            v-for="store in litterStores"
            :key="store.id"
      ...
      <view class="picker-content key-picker-content" @click.stop>
        ...
        <view class="key-method-list" style="padding: 24rpx 0 48rpx;">
          <view
            ...
            :key="opt"
            :class="['key-method-item', { 'selected': (keyPickerMode === 'handover' ? formData.keyHandoverMethod : formData.keyReturnMethod) === opt }]"
...
export default {
  ...
  data() {
    return {
      ...
      showKeyPicker: false,
      keyPickerMode: 'handover', // 'handover' | 'return'
      timeSlots: [],
      defaultTimeSlots: [
        '08:00-09:00',
        '09:00-10:00',
  ...
  computed: {
    ...
    storeSelectorLabel() {
      ...
      if (this.litterStores.length === 0) return '暂无门店（请在管理后台添加）'
      return '请选择上门铲屎门店'
    },
    formattedPrice() {
  ...
  onShow() {
    ...
    if (now - (this.lastOnShowLoadTime || 0) < throttleMs && (this.lastOnShowLoadTime || 0) > 0) return
    this.loadPets(true)
    if (this.formData.date) this.loadAvailableTimeSlots(true)
  ...
  methods: {
    ...
    formatDate(date) {
      ...
      // 与后端一致：时段开始时间须不早于「现在+ 2 小时」（跨日同样按绝对时间）
    ...
    openPetPicker() {
      if (this.pets.length === 0) this.loadPets(true)
    ...
    onDateChange(e) {
      ...
      this.formData.timeSlot = '' // 清空已选择的时间段
      // 加载该日期的可用时间段
        ...
        if (res.code === 200 || res.code === 0) {
          ...
          if (data.length > 0) {
            this.timeSlots = this.filterSlotsByMinAdvanceHours(data.map(slot => ({
              ...
              availableCount: (() => {
                if (slot.availableCount !== undefined && slot.availableCount !== null) return Number(slot.availableCount)
                if (slot.remainingCapacity !== undefined && slot.remainingCapacity !== null) return Number(slot.remainingCapacity)
                return undefined
              })()
            })))
          } else {
            // 该日期门店暂无排班（无人值班）时不再显示默认时段，避免用户误以为可预约
            this.timeSlots = []
          }
        } else {
          this.timeSlots = []
          uni.showToast({ title: res.msg || '加载时间段失败', icon: 'none' })
        }
      } catch (error) {
        console.error('加载可用时间段失败', error)
        this.timeSlots = []
        uni.showToast({ title: '加载时间段失败，请稍后重试', icon: 'none' })
      } finally {
    ...
    selectTimeSlot(slot) {
      const slotVal = typeof slot === 'string' ? slot : (slot && (slot.timeSlot || slot.value))
      const selectedSlot = this.timeSlots.find(s => (s.timeSlot || s.value) === slotVal)
      const disabled = selectedSlot && ((selectedSlot.status && selectedSlot.status !== '可预约') || (selectedSlot.availableCount !== undefined && selectedSlot.availableCount <= 0))
      if (selectedSlot && !disabled) {
        this.formData.timeSlot = slotVal || selectedSlot.timeSlot || selectedSlot.value
      } else if (disabled) {
        uni.showToast({ title: '该时间段暂不可用', icon: 'none' })
      }
    },
    ...
    async loadStores() {
      try {
        ...
        if ((res.code === 0 || res.code === 200) && res.data) {
          ...
          if (this.litterStores.length === 1) {
            this.formData.location = this.litterStores[0].address
            this.formData.storeId = this.litterStores[0].id
          } else if (this.litterStores.length > 1) {
            const defaultStore = this.litterStores.find(s => s.isDefault)
            if (defaultStore) {
    ...
    selectStore(store) {
      if (store.maxCapacity) {
        ...
        if (available <= 0) {
          uni.showToast({ title: '该门店已满，请选择其他门店', icon: 'none' })
          return
        }
      }
    ...
    selectKeyMethod(opt) {
      if (this.keyPickerMode === 'handover') {
        this.formData.keyHandoverMethod = opt
      } else {
        this.formData.keyReturnMethod = opt
      }
    ...
    validateForm() {
      ...
      if (!this.formData.storeId || !this.formData.location) {
        uni.showToast({
          title: '请选择服务门店',
          icon: 'none'
        })
      ...
      if (!this.formData.serviceAddress || this.formData.serviceAddress.trim() === '') {
        uni.showToast({
          title: '请输入上门服务地址',
          icon: 'none'
        })
    ...
    async submitBooking() {
      ...
      const serviceName = this.serviceName || '上门铲屎服务'
      ...
      const appointmentData = {
        ...
        location: this.formData.serviceAddress,
        keyHandoverMethod: this.formData.keyHandoverMethod || undefined,
        keyReturnMethod: this.formData.keyReturnMethod || undefined,
      ...
      const serviceBookingFormData = {
        ...
        appointmentData,
        successParams: {
          location: this.formData.serviceAddress || this.formData.location,
...
/* 宠物选择器、日期选择器、门店选择器（与洗护一致） */
.pet-selector, .date-picker, .store-selector {
  display: flex;
