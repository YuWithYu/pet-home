  ...
  <view class="booking-container">
    ...
    <view v-if="showStorePicker" class="picker-modal store-picker-modal" @click="showStorePicker = false">
      <view class="picker-content store-picker-content" @click.stop>
        <view class="picker-header">
          <view class="picker-title">选择洗护门店</view>
          <view class="picker-close" @click="showStorePicker = false">×</view>
        </view>
        <view class="picker-list">
          <view v-if="groomingStores.length === 0" class="empty-stores-hint">暂无门店，请在管理后台「服务管理」-「服务门店管理」中添加</view>
          <view
            v-for="store in groomingStores"
            :key="store.id"
          ...
          >
            ...
            <!-- 仅在有容量限制且需关注时显示：已满 / 仅剩X位；余位充足不显示-->
...
export default {
  ...
  computed: {
    ...
    storeSelectorLabel() {
      ...
      if (this.groomingStores.length === 0) return '暂无门店（请在管理后台添加）'
      return '请选择洗护门店'
    }
  },
  ...
  methods: {
    applyServiceOptions(options = {}) {
      ...
      const title = this.serviceName || '宠物洗护服务'
    ...
    async fetchServiceInfo(serviceId) {
      try {
        const res = await api.getGroomingServiceById(serviceId)
        if ((res.code === 0 || res.code === 200) && res.data) {
          if (!this.serviceName) {
            this.serviceName = res.data.name || ''
            uni.setNavigationBarTitle({
              title: this.serviceName || '宠物洗护服务'
            })
          }
    ...
    // 与后端一致：须提前至2 小时预约（按时段开始时间、绝对时刻判断）
    ...
    async loadAvailableTimeSlots(silent = false) {
      ...
      try {
        ...
        console.error('加载洗护可用时间段失败', error)
        this.timeSlots = []
        uni.showToast({
          title: '加载时间段失败，请稍后重试',
          icon: 'none'
    ...
    async loadStores() {
      try {
        const res = await api.getStoresByService('grooming')
        if ((res.code === 0 || res.code === 200) && res.data) {
          const storeList = Array.isArray(res.data) ? res.data : (res.data.records || res.data.list || [])
          this.groomingStores = storeList.map(store => {
          ...
          if (this.groomingStores.length === 1) {
            this.formData.location = this.groomingStores[0].address
            this.formData.storeId = this.groomingStores[0].id
          } else if (this.groomingStores.length > 1) {
            const defaultStore = this.groomingStores.find(s => s.isDefault)
            if (defaultStore) {
    ...
    async submitBooking() {
      ...
      const serviceName = this.serviceName || '宠物洗护服务'
      ...
      const serviceBookingFormData = {
        ...
        successParams: {
          location: this.formData.location,
          serviceType: this.formData.serviceType || 'grooming',
