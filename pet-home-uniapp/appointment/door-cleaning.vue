...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (response && (response.code === 200 || response.code === 0)) {
          ...
          this.services = records.map(service => ({
            ...
            image: util.getImageUrl(service.imageUrl || service.image_url || '/static/images/door-cleaning.svg')
          }))
        } else {
          console.error('获取铲屎服务失败:', response)
    ...
    onServiceTap(index) {
      ...
      uni.navigateTo({
        url: `/service/detail?serviceType=litter&serviceId=${service.id || index}`
      })
    },
