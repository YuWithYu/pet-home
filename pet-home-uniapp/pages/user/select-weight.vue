<template>
  <view class="select-weight-container">
    ...
    <!-- 页面内容 -->
    <view class="select-weight-content" :style="{ paddingTop: navBarTotalHeight + 'px' }">
      <!-- 主要内容容器（居中） -->
...
export default {
  ...
  methods: {
    ...
    complete() {
      ...
      // 创建完整的宠物档案
      const petProfile = {
        id: this.generatePetId(), // 生成在Integer范围内的ID
        avatar: basicInfo.avatar || '/static/images/garfield-default-avatar.png',
        name: basicInfo.name || '未命名',
        type: basicInfo.type || 'cat',
