<template>
  <view class="select-arrival-date-container">
    <!-- 自定义白色导航栏 -->
    ...
    <!-- 页面内容 -->
    <view class="select-arrival-date-content" :style="{ paddingTop: navBarTotalHeight + 'px' }">
      <!-- 主要内容容器（居中） -->
      <view class="main-content-container">
        <!-- 到家日期选择区域 -->
        <view class="arrival-date-selection-container">
          <view class="date-input-container" @click="showDatePicker">
            <text class="date-text" :class="{ 'placeholder': !selectedArrivalDate }">
              {{ selectedArrivalDate || '请选择宠物的到家日期' }}
            </text>
      ...
      <view class="next-button-section">
        <view 
          class="next-button" 
          :class="{ 'active': selectedArrivalDate }"
          @click="goToNext"
        >
          <text>下一步</text>
        </view>
    ...
    <view class="date-picker-modal" v-if="showDatePickerModal" @click="onDateCancel">
      <view class="date-picker-content" @click.stop>
        <view class="date-picker-header">
          <text class="date-picker-title">选择到家日期</text>
...
export default {
  ...
  methods: {
    ...
    confirmDate() {
      ...
      this.selectedArrivalDate = `${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`
...
/* 到家日期选择容器（居中） */
.arrival-date-selection-container {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 100%;
