<template>
  <view class="litter-service-container">
    <view class="service-header">
      ...
      <view v-else class="default-banner">
        ...
        <view class="service-title">上门铲屎服务</view>
        <view class="service-desc">专业贴心，让您的宠物生活更舒适</view>
      </view>
    </view>

    <view class="services-container">
      <view class="section-title">铲屎服务</view>
...
export default {
  ...
  onLoad() {
    this.loadLitterServices()
    this.loadLitterBanner()
  },

  methods: {
    async loadLitterServices() {
      try {
        this.loading = true
        console.log('开始请求铲屎服务API...')
        
        const [error, response] = await uni.request({
          url: 'http://localhost:8080/api/litter-services/page',
          method: 'GET',
          data: {
            pageNo: 1,
            pageSize: 20
        ...
        if (response && response.statusCode === 200 && response.data && response.data.code === 0) {
          ...
          this.services = records.map(service => ({
            ...
            image: service.imageUrl ? `http://localhost:8080${service.imageUrl}` : '/static/images/door-cleaning.svg'
          }))
          console.log('处理后的服务数据:', this.services)
        } else {
          console.error('获取铲屎服务失败 - 状态码:', response?.statusCode, '数据:', response?.data)
          this.loadDefaultServices()
        }
      } catch (error) {
        console.error('请求铲屎服务API失败:', error)
    ...
    async loadLitterBanner() {
      try {
        console.log('开始请求铲屎服务展示图API...')
        
        const [error, response] = await uni.request({
          url: 'http://localhost:8080/tz/litter-banner',
          method: 'GET',
          header: {
            'Content-Type': 'application/json'
          },
        ...
        if (response && response.statusCode === 200 && response.data && response.data.code === 0) {
          ...
          console.error('获取铲屎服务展示图失败 - 状态码:', response?.statusCode, '数据:', response?.data)
        }
      } catch (error) {
        console.error('请求铲屎服务展示图API失败:', error)
        console.error('错误详情:', JSON.stringify(error))
      }
    },
      ...
      this.services = [
        {
          ...
          price: 99,
          duration: 30,
          image: '/static/images/door-cleaning.svg'
        },
        {
          id: 2,
          title: '深度清洁服务',
    ...
    onServiceTap(service, index) {
      ...
      uni.navigateTo({
        url: `/pages/service/detail?serviceType=litter&serviceId=${service.id || index}`
      })
    },
