...
export const asyncRoutes = [
  ...
  // 我的评价（服务人员查看收到的用户评价）
  {
    ...
    redirect: "/my-reviews",
    name: "MyReviewsRoot",
    meta: { title: "我的评价", icon: "star", defaultOpen: false, permissions: ["staff"] },
    children: [
      {
        path: "",
        name: "MyReviews",
        component: () => import("@/views/vab/my-reviews.vue").catch(() => {
          console.error('我的评价组件加载失败');
          return import("@/views/404.vue");
        }),
        meta: {
          title: "我的评价",
...
export function setupRoutesByRole(role, addToRouter = true) {
  ...
  if (role === 'admin') {
    ...
    return asyncRoutes.filter((route) => ["MySchedule", "VerifyRoot", "MyReviewsRoot"].includes(route.name));
  } else {
    // 其他角色或未登录：不加载路由
