  ...
  <div class="grooming-service-container">
    <el-card shadow="never" class="main-card">
      ...
      <el-tabs v-model="activeTab" class="inner-tabs" @tab-change="handleTabChange">
        <el-tab-pane v-if="!isStaff && !showOrderTabs" label="所有洗护服务" name="services">
          <el-table
            :data="services"
        ...
        <el-tab-pane v-if="!isStaff && !showOrderTabs" label="服务展示图管理" name="banner">
          <div class="banner-section">
            <h4>洗护服务展示图</h4>
            <p class="banner-desc">展示图会显示在小程序宠物洗护服务页顶部，推荐尺寸 750×400</p>
        ...
        <el-tab-pane v-if="!isStaff && showOrderTabs" label="洗护预约订单" name="orders">
    ...
    <el-dialog
      ...
      :title="isEditService ? '编辑洗护服务' : '添加洗护服务'"
      width="640px"
    ...
    >
      <el-descriptions :column="1" border>
        <el-descriptions-item label="洗护服务总数">
...
const BANNER_POSITION = 'grooming-page-top'
...
const serviceTypeNav = ref('grooming')
...
const syncServiceTypeNav = () => {
  if (route.path.includes('/hospital-service')) {
...
const loadServices = async () => {
  ...
  try {
    ...
    const res = await groomingServiceApi.getGroomingServicePage(params)
    if (isSuccess(res) && res.data) {
      ...
      ElMessage.error(resolveMessage(res, '加载洗护服务失败'))
    }
  } catch (error) {
    console.error('加载洗护服务失败:', error)
    services.value = []
    servicePagination.total = 0
    ElMessage.error('加载洗护服务失败，请稍后重试')
  } finally {
...
const openEditServiceDialog = async (row) => {
  try {
    const res = await groomingServiceApi.getGroomingServiceById(row.id)
    ...
    console.error('加载洗护服务详情失败:', error)
...
const saveService = async () => {
  ...
  try {
    ...
    if (isEditService.value) {
      res = await groomingServiceApi.updateGroomingService(payload)
    } else {
      res = await groomingServiceApi.createGroomingService(payload)
    }
    if (isSuccess(res)) {
...
const toggleServiceStatus = async (row) => {
  ...
  try {
    const res = await groomingServiceApi.updateGroomingServiceStatus(row.id, next)
    if (isSuccess(res)) {
...
const deleteService = async (row) => {
  try {
    const res = await groomingServiceApi.deleteGroomingService(row.id)
    if (isSuccess(res)) {
...
const uploadUrl = ref(apiBase + '/api/grooming-services/upload')
const bannerUploadUrl = ref(apiBase + '/api/grooming-banners/upload')
...
const loadServiceBanner = async () => {
  try {
    const res = await groomingServiceApi.getServiceBanner(BANNER_POSITION)
    if (isSuccess(res) && res.data) {
  ...
  ElMessageBox.confirm('确定要删除当前展示图吗？', '提示', {
    ...
    .then(async () => {
      try {
        let res
        if (bannerId) {
          res = await groomingServiceApi.deleteServiceBanner(bannerId)
        } else {
          serviceBannerRecord.value = null
          serviceBannerImage.value = ''
...
const loadServiceMembers = async () => {
  ...
  try {
    ...
    console.error('加载洗护服务人员失败:', error)
    serviceMembers.value = []
  }
}
...
const loadServiceOrders = async () => {
  ...
  try {
    ...
    console.error('加载洗护预约订单失败:', error)
    serviceOrders.value = []
    orderPagination.total = 0
  } finally {
...
const loadOrderServiceStores = async () => {
  try {
    ...
    if (isSuccess(res) && res.data && Array.isArray(res.data)) {
      ...
      if (currentStoreId.value != null && orderFilterStoreId.value == null) {
        orderFilterStoreId.value = currentStoreId.value
      }
    } else {
...
const viewOrderDetail = async (order) => {
  ...
  try {
    const res = await api.get(`/api/grooming-appointments/${order.id}`)
    if (isSuccess(res) && res.data) {
...
const updateAppointmentStatus = async (order, status, successMessage, rejectReason = null) => {
  try {
    ...
    const res = await api.put(`/api/grooming-appointments/${order.id}/status`, null, {
      params,
    })
    if (isSuccess(res)) {
  ...
  try {
    ...
    if (!member) {
      ...
      if (isSuccess(listRes) && Array.isArray(listRes.data)) {
        member = listRes.data.find((item) => item.userId === userId || item.username === currentUsername.value) || null
      }
    }
    if (member) {
    ...
    console.error('获取洗护服务人员信息失败:', error)
  }
}
...
const loadMyOrders = async () => {
  ...
  try {
    const res = await api.get('/api/grooming-appointments/member/list', {
...
// 监听标签页切换，切换到排班时超级管理员可刷新门店列表与日历
const handleTabChange = (tabName) => {
  if (tabName === 'schedule' && isSuperAdmin.value) {
