<template>
  <div class="hospital-service-container">
    <el-card shadow="never" class="main-card">
      ...
      <el-tabs v-model="activeTab" class="inner-tabs" @tab-change="handleTabChange">
        <el-tab-pane v-if="!isStaff && !showOrderTabs" label="所有医院服务" name="services">
          <el-table
            :data="services"
          ...
          >
            ...
            <el-table-column label="操作" width="220" fixed="right">
              <template #default="{ row }">
                <el-button type="text" size="small" @click="openEditServiceDialog(row)">编辑</el-button>
                <el-button type="text" size="small" @click="openImageDialog(row)">更换图片</el-button>
                <el-button
                  type="text"
                  size="small"
            ...
            <div class="schedule-table-wrapper">
              ...
              >
                ...
                <el-table-column label="操作" min-width="140">
                  <template #default="{ row }">
                    <el-button type="text" size="small" @click="openScheduleEditDialog(row)">编辑</el-button>
                    <el-popconfirm
                      title="确定删除该排班吗？"
          ...
          <div class="time-slot-panel">
            ...
            >
              ...
              <el-table-column label="操作" width="200">
                <template #default="{ row }">
                  <el-button type="text" size="small" @click="editTimeSlot(row)">编辑</el-button>
                  <el-button
                    type="text"
                    size="small"
        ...
        <el-tab-pane v-if="!isStaff && !showOrderTabs" label="服务展示图管理" name="banner">
          <div class="banner-section">
            <h4>医院服务展示图</h4>
            <p class="banner-desc">展示图会显示在小程序宠物医院服务页顶部，推荐尺寸 750×400</p>
        ...
        <el-tab-pane v-if="!isStaff && showOrderTabs" label="医院预约订单" name="orders">
              ...
              <el-table-column label="操作" width="220" fixed="right">
                <template #default="{ row }">
                  <span class="order-actions-inline">
                    <el-button type="text" size="small" @click="viewOrderDetail(row)">详情</el-button>
                    <el-button
    ...
    <el-dialog
      ...
      :title="isEditService ? '编辑医院服务' : '添加医院服务'"
      width="640px"
    ...
    >
      ...
      >
        ...
        <el-form-item label="服务人员" prop="memberId">
          <el-select v-model="scheduleEditForm.memberId" placeholder="选择医生/护士">
            <el-option
              v-for="member in serviceMembers"
              :key="member.id"
              :label="member.memberName || member.username"
    ...
    >
      ...
      >
        ...
        <el-descriptions-item label="宠物">{{ currentOrderDetail.petName || '-' }}</el-descriptions-item>
        ...
        <el-descriptions-item label="服务人员">
          <span v-if="currentOrderDetail.memberName">{{ currentOrderDetail.memberName }}</span>
          <span v-else class="assign-member-inline">
            <el-select
            ...
            >
              <el-option
                v-for="m in assignMemberList"
                :key="m.id"
                :label="m.memberName || m.username || `成员${m.id}`"
                :value="m.id"
              />
            </el-select>
            <el-button type="primary" size="small" :loading="assignLoading" @click="submitAssignMember">
              分配
            </el-button>
          </span>
        </el-descriptions-item>
        <el-descriptions-item label="就诊门店" :span="2">
    ...
    <!-- 取消预约审核弹窗 -->
    <el-dialog v-model="cancelReviewModalVisible" title="取消预约审核" width="480px" :close-on-click-modal="false" @closed="cancelReviewRequest = null">
    ...
    <!-- 预约变更审核弹窗 -->
    <el-dialog v-model="changeReviewModalVisible" title="预约变更审核" width="500px" :close-on-click-modal="false" @closed="changeReviewRequest = null">
    ...
    >
      <el-descriptions :column="1" border>
        <el-descriptions-item label="医院服务总数">
...
const SERVICE_TYPE = 'hospital'
const BANNER_POSITION = 'hospital-page-top'
...
const serviceTypeNav = ref('hospital')
...
const syncServiceTypeNav = () => {
  ...
  } else if (route.path.includes('/litter-service')) {
...
const loadServices = async () => {
  ...
  try {
    ...
    const res = await hospitalServiceApi.getHospitalServicePage(params)
    if (isSuccess(res) && res.data) {
      ...
      ElMessage.error(resolveMessage(res, '加载医院服务失败'))
    }
  } catch (error) {
    console.error('加载医院服务失败:', error)
    services.value = []
    servicePagination.total = 0
    ElMessage.error('加载医院服务失败，请稍后重试')
  } finally {
...
const openEditServiceDialog = async (row) => {
  try {
    const res = await hospitalServiceApi.getHospitalServiceById(row.id)
    ...
    console.error('加载医院服务详情失败:', error)
...
const saveService = async () => {
  ...
  try {
    ...
    if (isEditService.value) {
      res = await hospitalServiceApi.updateHospitalService(payload)
    } else {
      res = await hospitalServiceApi.createHospitalService(payload)
    }
    if (isSuccess(res)) {
...
const toggleServiceStatus = async (row) => {
  ...
  try {
    const res = await hospitalServiceApi.updateHospitalServiceStatus(row.id, next)
    if (isSuccess(res)) {
...
const deleteService = async (row) => {
  try {
    const res = await hospitalServiceApi.deleteHospitalService(row.id)
    if (isSuccess(res)) {
...
const uploadUrl = ref(apiBase + '/api/hospital-services/upload')
const bannerUploadUrl = ref(apiBase + '/api/hospital-banners/upload')
...
const loadServiceBanner = async () => {
  try {
    const res = await hospitalServiceApi.getServiceBanner(BANNER_POSITION)
    if (isSuccess(res) && res.data) {
  ...
  ElMessageBox.confirm('确定要删除当前展示图吗？', '提示', {
    ...
    .then(async () => {
      try {
        let res
        if (bannerId) {
          res = await hospitalServiceApi.deleteServiceBanner(bannerId)
        } else {
          serviceBannerRecord.value = null
          serviceBannerImage.value = ''
...
const loadServiceMembers = async () => {
  ...
  try {
    ...
    console.error('加载医院服务人员失败:', error)
    serviceMembers.value = []
  }
}
...
const generateBatchSchedule = async () => {
  ...
  try {
    ...
    if (isSuccess(res)) {
      ...
      if (count > 0) {
        ElMessage.success(`排班生成成功，共 ${count} 条`)
      } else {
        ElMessage.warning('排班生成完成，本次新增 0 条。若所选日期与时段已有排班则不会重复生成；请确认服务人员为「医院」类型且已启用。')
      }
...
const APPOINTMENT_TYPE = 'hospital'
...
const loadServiceOrders = async () => {
  ...
  try {
    const res = await api.get('/api/hospital-appointments/page', {
      params: {
    ...
    if (isSuccess(res) && res.data) {
      ...
      console.error('加载医院预约订单失败:', res)
      ElMessage.error(errorMsg)
    }
  } catch (error) {
    console.error('加载医院预约订单失败:', error)
    serviceOrders.value = []
    orderPagination.total = 0
...
const assignMemberList = ref([])
const assignLoading = ref(false)

const loadAssignMemberList = async (storeId) => {
  try {
    const params = { serviceType: SERVICE_TYPE }
    if (storeId != null) params.storeId = storeId
    const res = await api.get('/api/service-member/list', { params })
    assignMemberList.value = isSuccess(res) && Array.isArray(res.data) ? res.data : []
  } catch (e) {
...
const submitAssignMember = async () => {
  if (!currentOrderDetail.value?.id || !assignMemberId.value) {
    ElMessage.warning('请选择服务人员')
    return
  }
  assignLoading.value = true
  try {
    const res = await api.put(`/api/hospital-appointments/${currentOrderDetail.value.id}/assign`, null, {
      params: { memberId: assignMemberId.value },
    })
    if (isSuccess(res)) {
      ElMessage.success('分配成功')
      const member = assignMemberList.value.find((m) => m.id === assignMemberId.value)
      if (member) {
        currentOrderDetail.value.memberName = member.memberName || member.username || ''
      }
      loadServiceOrders()
      if (isStaff.value) loadMyOrders()
    } else {
      ElMessage.error(resolveMessage(res, '分配失败'))
    }
  } catch (e) {
    ElMessage.error(e?.response?.data?.msg || e?.message || '分配失败')
  } finally {
...
const openCancelReviewModal = async (row) => {
  ...
  try {
    ...
    if (!cancelReviewRequest.value) ElMessage.warning('未查到该预约的取消申请或已处理')
  } catch (e) {
...
const openChangeReviewModal = async (row) => {
  ...
  try {
    ...
    if (!changeReviewRequest.value) ElMessage.warning('未查到该预约的变更申请或已处理')
  } catch (e) {
    ElMessage.error(e?.message || e?.msg || '加载失败')
...
const viewOrderDetail = async (order) => {
  ...
  assignMemberId.value = null
  try {
    const res = await api.get(`/api/hospital-appointments/${order.id}`)
    if (isSuccess(res) && res.data) {
    ...
    // 当无服务人员时加载可分配人员列表
    if (!currentOrderDetail.value?.memberName) {
      await loadAssignMemberList(currentOrderDetail.value?.storeId ?? order?.storeId)
    }
  } catch (error) {
...
const updateAppointmentStatus = async (order, status, successMessage, rejectReason = null) => {
  try {
    const params = { status }
    if (status === 'cancelled') {
      params.forceCancelAfterServiceStart = true
    }
    if (rejectReason != null && String(rejectReason).trim() !== '') {
      params.rejectReason = String(rejectReason).trim()
    }
    const res = await api.put(`/api/hospital-appointments/${order.id}/status`, null, {
      params,
    })
    if (isSuccess(res)) {
  ...
  try {
    ...
    if (userId) {
      try {
        ...
        if (isSuccess(res) && res.data && res.data.serviceType === SERVICE_TYPE) {
          member = res.data
    ...
    if (!member) {
      try {
        const listRes = await api.get('/api/service-member/list', { params: { serviceType: SERVICE_TYPE } })
        if (isSuccess(listRes) && Array.isArray(listRes.data)) {
          ...
          if (!member && userDepartment && userDepartment.includes('医院')) {
    ...
    console.error('获取医院服务人员信息失败:', error)
    myMemberId.value = null
    myMemberName.value = ''
  }
}
...
const loadMyOrders = async () => {
  ...
  try {
    const res = await api.get('/api/hospital-appointments/member/list', {
...
const transformAppointmentRecord = (record) => {
  ...
  return {
    ...
    serviceType: record.serviceType || SERVICE_TYPE,
    date,
...
.medical-record-item {
  ...
  border-radius: 8px;
  margin-bottom: 8px;
  border-left: 3px solid #8D9F5E;
}
.medical-record-item .mr-date {
  color: #666;
  margin-right: 8px;
}
.medical-record-item .mr-type {
  font-size: 12px;
  color: #8D9F5E;
  background: #e8f0e8;
  padding: 2px 8px;
...
.medical-record-item .mr-name {
  font-weight: 500;
  color: #333;
}
.medical-record-item .mr-next,
.medical-record-item .mr-remark {
  display: block;
  font-size: 12px;
  color: #666;
  margin-top: 4px;
