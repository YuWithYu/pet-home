          ...
          <div class="orders-panel">
            <div class="orders-toolbar">
              ...
              >
                ...
                <el-option
                  v-for="name in orderServiceNameOptions"
                  :key="'service-' + name"
                  :label="name"
                  :value="name"
                />
...
const orderServiceNameOptions = computed(() =>
  Array.from(new Set((serviceOrders.value || []).map((x) => x.serviceName).filter(Boolean)))
)
const filteredServiceOrders = computed(() => {
  if (!orderFilterServiceName.value) return serviceOrders.value
  return (serviceOrders.value || []).filter((x) => x.serviceName === orderFilterServiceName.value)
})
